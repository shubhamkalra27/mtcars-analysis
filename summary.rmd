...
One may also run `summary(cars)` to get more interesting statistics about each variable. We observe that few variables such as 'Number of Cylinders', 'Number of forward gears', etc. are categorical variables. We convert those to [factor variables](http://www.stat.berkeley.edu/~s133/factors.html).
...
* Intercepts value means that Automatic Transmission has **`r round(summary(model1)$coefficients[1],3)`** average MPG  
* Coefficient value of `am` says that Manual transmission cars add **`r round(summary(model1)$coefficients[2],3)`** MPG's more to the Automatic's mean value  
...
Next, we would like to see dependence of other variables on Mileage. To find out which preditor variables to use in our model, we run a Stepwise regression to identify useful subset of predictors. 
...
* The variables mentioned by bestModel are non-collinear and most useful set of predictors
